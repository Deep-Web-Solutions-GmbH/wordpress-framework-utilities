	...
	protected function register_missing_dependencies_admin_notices( AdminNoticesService $notices_service, DependenciesCheckerInterface $dependencies_checker ): void {
		...
		foreach ( $missing_dependencies as $type => $handler_dependencies ) {
			foreach ( $handler_dependencies as $handler => $dependencies ) {
				...
				switch ( $type ) {
					case 'php_extensions':
						$message = $this->compose_missing_php_extensions_message( $dependencies, $is_optional_handler );
						break;
					case 'php_functions':
						$message = $this->compose_missing_php_functions_message( $dependencies, $is_optional_handler );
						break;
					case 'php_settings':
						$message = $this->compose_incompatible_php_settings_message( $dependencies, $is_optional_handler );
						break;
					case 'active_plugins':
						$message = $this->compose_missing_plugins_message( $dependencies, $is_optional_handler );
						break;
					default:
						throw new NotImplementedException( 'Dependencies admin notices scenario not supported' );
				}
				...
				$notice = $is_optional_handler
					? new DismissibleNotice( $notice_handle, $message, $notice_params )
					: new Notice( $notice_handle, $message, $notice_params );
